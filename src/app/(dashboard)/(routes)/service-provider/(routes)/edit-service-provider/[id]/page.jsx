...
export default function AddAdmin({ params }) {
    ...
    const { data: serviceProvider, isLoading: serviceProviderLoading, isError: serviceProviderError } = useQuery({
        queryKey: ["serviceProvider"],
        queryFn: async() => await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/serviceprovider/${params.id}`).then((res) => {
    ...
    const handleAddServiceProvider = (e) => {
        ...
        axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/serviceprovider/update/${params.id}`, formData)
            .then((res) => {
                console.log(res);
                if (res.data.success) {
...
//console.log(formData)
    return (
        ...
        <h1 className="text-2xl font-semi">Edit Service Provider</h1>
                            ...
                            <form onSubmit={handleAddServiceProvider} className="flex flex-col gap-4">
                                <div>
                                    ...
                                    {services && services.length > 0 &&
                                        <Select onValueChange={(value) => setFormData({ ...formData, serviceId: value })} value={formData.serviceId}>
                                            <SelectTrigger>
                                ...
                                <div >
                                    ...
                                    {vehicleTypes && vehicleTypes.length > 0 &&
                                        <Select onValueChange={(value) => setFormData({ ...formData, vehicleTypeId: value })} value={formData.vehicleTypeId}>
                                            <SelectTrigger>
                                ...
                                <div>
                                    ...
                                    <Input name="price" type="number" value={formData.price} onChange={handleChange} placeholder="Type price here..." required />
                                </div>
                                <div>
                                    <Label htmlFor="duration">Duration(hrs)</Label>
                                    <Input name="duration" type="number" value={formData.duration} onChange={handleChange} placeholder="Type duration here..." required />
                                </div>
